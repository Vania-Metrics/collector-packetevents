...
rootProject.name = "colecteur-packetevents"

dependencyResolutionManagement {
    repositoriesMode = RepositoriesMode.FAIL_ON_PROJECT_REPOS
    repositories {
        // Dated snapshots (paper-api, spark-api) are read via an ivy repo, as
        // fixed versions. Through the Maven repo, Gradle treats them as SNAPSHOT,
        // and its checksum verification breaks while writing
        // verification-metadata.xml — or even writes an incomplete file
        // (gradle/gradle#32739, #26803, open). The cost: no transitive
        // dependencies. The catalog's "paper" bundle declares them.
        exclusiveContent {
            forRepository {
...
val vaniaCoreDir: File = providers.gradleProperty("vaniaCore.dir").orNull?.let { file(it) } ?: run {
    ...
    if (!dir.resolve(".git").exists()) {
        ...
        // TODAY, whether it's a tag or a branch. Offline, keep the clone as long
        // as it's at the right ref — otherwise refuse, rather than silently
        // compiling against another API version.
